...
- **Resting** (`{ kind: "resting" }`; `retryAfter` from the activity endpoint, 409 on an attempt): a task a learner answered less than `TASK_REST_MS` ago, which they may not answer again yet, since grading showed them the answer ([ADR 0017](adr/0017-task-rest.md)). When every task of the decided objective rests, the learner waits rather than being offered another objective.
