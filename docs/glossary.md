...
- **Task** (`task`, `taskId`): a single learner interaction within an activity, such as answering a question, assessing one objective. Its `kind` decides its shape and grading; `choice` is the first. Immutable: correcting one creates another and retires the wrong one, which is then never offered or answered again ([ADR 0015](adr/0015-tasks.md)).
