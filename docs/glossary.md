...
- **Task** (`task`, `taskId`): a single learner interaction within an activity, such as answering a question, assessing one objective. Its `kind` decides its shape and grading; `choice` is the first. Immutable: correcting one creates another ([ADR 0015](adr/0015-tasks.md)).
- **Attempt** (`attempt`): a learner's response to a task, which Braivo grades into evidence. It references an immutable task, which is what keeps it interpretable after content changes. Its ID is the client's, so a resubmission is recorded once.
