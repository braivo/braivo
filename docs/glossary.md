...
- **Task** (`task`, `taskId`): a single learner interaction within an activity, such as answering a question, assessing one objective. Its `kind` decides its shape and grading; `choice` is the first. Immutable: correcting one creates another and retires the wrong one ([ADR 0015](adr/0015-tasks.md)).
...
- **Learner progress** (`LearnerProgress`; the `…/progress` endpoint, which answers a `KnowledgeReport`): where one learner stands in one course, for that learner and the content owners of its organization.
