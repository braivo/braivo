...
- **The actor comes from the session, never from the request.** Whoever is asking is resolved from Better Auth, and no endpoint names them. A learner who is being _written about_ or _read about_ is named in the path, because they are somebody other than the actor — progress and evidence both do — and naming them is what there is to authorize. `next` has no such need: its learner is the actor, so there is nothing to authorize and nothing to get wrong.
