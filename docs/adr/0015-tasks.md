...
- **Braivo grades.** The learner posts a response to `POST /api/courses/:courseId/attempts`; the server grades it and writes the attempt and its evidence in one transaction. A learner may submit for themselves because they choose the answer, never the outcome. The administrator-only evidence endpoint remains for integrators who grade elsewhere.
...
- AI-graded kinds break the "recompute the grade" rule, since a model's verdict is not reproducible. They will store the grade and its provenance (model, prompt version) on the attempt when they arrive.
